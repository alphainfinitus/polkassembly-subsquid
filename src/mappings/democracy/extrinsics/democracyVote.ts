...
import {
  getDelegations,
  removeDelegatedVotesReferendum,
  addDelegatedVotesReferendum,
} from "./utils";
...
export async function handleVote(
  ...
  const proposal = await ctx.store.get(Proposal, {
    where: { index, type: ProposalType.Referendum },
  });
  if (!proposal) {
  ...
  if (votes) {
    ...
    if (votes.length > 0) {
      ...
      const flattenedVote = await ctx.store.get(FlattenedConvictionVotes, {
        where: {
          voter: from,
          proposalIndex: index,
          removedAtBlock: IsNull(),
      ...
      if (delegatedVotes) {
        await removeDelegatedVotesReferendum(
          ctx,
          header.height,
          header.timestamp,
          delegatedVotes
  ...
  if (vote.type === "Split") {
    ...
    if (lockPeriod === 0 && vote.balance) {
      ...
      votingPower =
        lockPeriod && vote.balance
          ? vote.balance * BigInt(lockPeriod)
          : BigInt(0);
    }
  }
  let flattenedVotes = [];
  ...
  if ([VoteDecision.yes, VoteDecision.no].includes(decision)) {
    const { delegatedVotesNested, delegatedVotePower, flattenedVotesNested } =
      await addDelegatedVotesReferendum(
        ctx,
        header.height,
        header.timestamp,
        nestedDelegations,
    ...
    convictionVote.delegatedVotingPower = convictionVote.delegatedVotingPower
      ? convictionVote.delegatedVotingPower + delegatedVotePower
      : delegatedVotePower;
    convictionVote.totalVotingPower =
      votingPower + convictionVote.delegatedVotingPower;
