...
function getEventData(itemEvent: Event): ReferendumEventData {
  if (started.v1013.is(itemEvent)) {
    const { refIndex: index, threshold } = started.v1013.decode(itemEvent);
    return {
      index,
...
export async function handleStarted(
  ...
  const storageData = await storage.democracy.getReferendumInfoOf(
    ctx,
    index,
    header
  );
  ...
  if (storageData.status === "Finished") {
    ctx.log.warn(
      `Referendum with index ${index} has already finished at block ${header.height}`
    );
    return;
  }
