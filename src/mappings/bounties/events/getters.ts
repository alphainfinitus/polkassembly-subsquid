...
export function getBountyAwardedData(itemEvent: Event): BountyAwardedData {
    if (bountyAwarded.v1001002.is(itemEvent)) {
        const { index, beneficiary } = bountyAwarded.v1001002.decode(itemEvent)
        return {
            index,
            beneficiary,
        }
...
export function getBountyBacameActiveData(itemEvent: Event): BountyBacameActiveData {
    if (bountyBecameActive.v1001002.is(itemEvent)) {
        const { index } = bountyBecameActive.v1001002.decode(itemEvent)
        return {
            index,
        }
    } else {
...
export function getBountyCanceledData(itemEvent: Event): BountyCanceledData {
    if (bountyCanceled.v1001002.is(itemEvent)) {
        const { index } = bountyCanceled.v1001002.decode(itemEvent)
        return {
            index,
        }
    } else {
...
export function getBountyClaimedData(itemEvent: Event): BountyClaimedData {
    if (bountyClaimed.v1001002.is(itemEvent)) {
        const { index, payout, beneficiary } = bountyClaimed.v1001002.decode(itemEvent)
        return {
            index,
            payout,
            beneficiary,
...
export function getBountyExtendedData(itemEvent: Event): BountyExtendedData {
    if (bountyExtended.v1001002.is(itemEvent)) {
        const { index } = bountyExtended.v1001002.decode(itemEvent)
        return {
            index,
        }
    } else {
...
export function getBountyProposedData(itemEvent: Event): BountyProposedData {
    if (bountyProposed.v1001002.is(itemEvent)) {
        const { index } = bountyProposed.v1001002.decode(itemEvent)
        return {
            index,
        }
    } else {
...
export function getBountyRejectedData(itemEvent: Event): BountyRejectedData {
    if (bountyRejected.v1001002.is(itemEvent)) {
        const { index } = bountyRejected.v1001002.decode(itemEvent)
        return {
            index,
        }
    } else {
