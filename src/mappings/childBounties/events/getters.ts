...
export function getChildBountyAwardedData(itemEvent: any): ChildBountyAwardedData {
    if (awarded.v1001002.is(itemEvent)) {
        const { index, childIndex, beneficiary } = awarded.v1001002.decode(itemEvent)
        return {
            parentIndex: index,
            childIndex,
            beneficiary,
...
export function getChildBountyCancelledData(itemEvent: any): ChildBountyCancelledData {
    if (canceled.v1001002.is(itemEvent)) {
        const { index, childIndex } = canceled.v1001002.decode(itemEvent)
        return {
            parentIndex: index,
            childIndex
        }
...
export function getChildBountyClaimedData(itemEvent: any): ChildBountyClaimedData {
    if (claimed.v1001002.is(itemEvent)) {
        const { index, childIndex, payout, beneficiary } = claimed.v1001002.decode(itemEvent)
        return {
            parentIndex: index,
            childIndex,
            payout,
...
export function getChildBountyAddedData(itemEvent: any): ChildBountyAddedData {
    if (added.v1001002.is(itemEvent)) {
        const { index, childIndex } = added.v1001002.decode(itemEvent)
        return {
            parentIndex: index,
            childIndex
        }
