...
export function getApprovedData(itemEvent: Event): string {
    if (approved.v5000.is(itemEvent)) {
        return approved.v5000.decode(itemEvent).proposalHash
    } else {
...
export function getClosedData(itemEvent: Event): string {
    if (closed.v5000.is(itemEvent)) {
        return closed.v5000.decode(itemEvent).proposalHash
    } else {
        throw new UnknownVersionError(itemEvent.name)
    }
}
...
export function getDisapprovedData(itemEvent: Event): string {
    if (disapproved.v5000.is(itemEvent)) {
        return disapproved.v5000.decode(itemEvent).proposalHash
    } else {
        throw new UnknownVersionError(itemEvent.name)
    }
}
...
export function getProposedData(itemEvent: Event): ProposedData {
    if (proposed.v5000.is(itemEvent)) {
        const { account, proposalIndex, proposalHash, threshold } = proposed.v5000.decode(itemEvent)
        return {
            proposer: account,
            index: proposalIndex,
            hash: proposalHash,
...
export function getVotedData(itemEvent: Event): VotedData {
    if (voted.v5000.is(itemEvent)) {
        const { account, proposalHash, voted: voteData } = voted.v5000.decode(itemEvent)
        return {
            voter: account,
            hash: proposalHash,
            decision: voteData,
