...
async function getStorageData(
  ...
  const preimageStatus: PreimageStatusStorageData | undefined =
    await getPreimageStatusData(ctx, hash, block);
  if (preimageFor.v1013.is(block)) {
    if (preimageStatus && preimageStatus.len) {
      const storageData = await preimageFor.v1013.get(block, [
        hash,
        preimageStatus.len,
      ]);
...
export async function handlePreimageV2Noted(
  ctx: ProcessorContext<Store>,
  item: Event,
  header: any
) {
  ...
  try {
    ...
    console.log(
      `Error decoding call ${header.height}, extrinsicIndex: ${item.extrinsicIndex} ${e}`
    );
    return;
  }
  ...
  const desc = (
    item.block._runtime.calls.get(`${section}.${method}`).docs as string[]
  ).join("\n");
  ...
  const proposer = storageData.value
    ? ss58codec.encode(decodeHex(value[0]))
    : undefined;
