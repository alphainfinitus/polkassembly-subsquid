...
import { ConvictionDelegatedVotes, ConvictionVote, StandardVoteBalance, VoteType, VotingDelegation, FlattenedConvictionVotes, DelegationType, ProposalType, Proposal } from '@model/index'
 ...
 * This is used to lazily create referendum records for referenda that were
 * migrated from the relay chain but don't yet exist in the squid database.
 */
 ...
 * 
 * This function handles the race condition between Kusama relay chain processor
 * and AssetHub processor after the AssetHub migration. Referenda that were created
 * on the relay chain but whose voting is now happening on AssetHub may not exist
 * in the database yet if the Kusama processor hasn't caught up.
 * 
 * This function:
...
export async function getOrCreateReferendumV2(
    ...
    // This handles migrated referenda that haven't been indexed yet
