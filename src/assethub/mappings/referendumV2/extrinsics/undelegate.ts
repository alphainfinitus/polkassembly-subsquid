...
import { NoDelegationFound, TooManyOpenDelegations, TooManyOpenVotes } from '@shared/errors'
import { IsNull } from 'typeorm'
import { ConvictionDelegatedVotes, ConvictionVote, DelegationType, Proposal, ProposalType, VoteType } from '@model/index'
import { getUndelegateData } from '@assethub/mappings/referendumV2/extrinsics/getters'
import { Store } from '@subsquid/typeorm-store'
import {
    VotingDelegation
} from '@model/index'
import { removeFlattenedVotes } from '@assethub/mappings/referendumV2/extrinsics/utils'
import { Call, ProcessorContext } from '@src/processor'
import { sendGovEvent } from '@assethub/mappings/utils/proposals'
        ...
        if (delegation && referendum) {
            ...
            if (votes) {
                ...
                if (votes.length > 0) {
                    ...
                    if (delegatedVotes) {
                        for (let j = 0; j < delegatedVotes?.length; j++) {
                            const delegatedVote = delegatedVotes[j]
                            if (delegatedVote) {
