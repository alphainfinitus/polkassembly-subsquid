...
import { delegate, removeOtherVote, removeVote, undelegate, vote } from '@assethub/types/conviction-voting/calls'
import { convictionToLockPeriod } from '@assethub/mappings/referendumV2/extrinsics/utils'
...
export function getVoteData(itemCall: any): DemocracyVoteCallData {
    if (vote.v2000000.is(itemCall)) {
        const { pollIndex, vote: voteData } = vote.v2000000.decode(itemCall)
...
export function getDelegateData(itemCall: any): ConvictionVoteDelegateCallData {
    if (delegate.v2000000.is(itemCall)) {
        //{ class, to, conviction, balance}
        const eventData = delegate.v2000000.decode(itemCall)
        return {
...
export function getUndelegateData(itemCall: any): ConvictionVoteUndelegateCallData {
    if (undelegate.v2000000.is(itemCall)) {
        const eventData = undelegate.v2000000.decode(itemCall)
        return {
            track: eventData.class
        }
    } else {
...
export function getRemoveVoteData(itemCall: any): ConvictionVotingRemoveVoteCallData {
    if (removeVote.v2000000.is(itemCall)) {
        const eventData = removeVote.v2000000.decode(itemCall)
        return {
...
export function getRemoveOtherVoteData(itemCall: any): ConvictionVotingRemoveOtherVoteCallData {
    if (removeOtherVote.v2000000.is(itemCall)) {
        const eventData = removeOtherVote.v2000000.decode(itemCall)
        return {
            index: eventData.index,
            track: eventData.class,
