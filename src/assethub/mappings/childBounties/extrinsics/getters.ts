...
export function getAccepterCuratorData(itemCall: any): AccepterCuratorData {
    if (acceptCurator.v2000000.is(itemCall)) {
        const { parentBountyId, childBountyId } = acceptCurator.v2000000.decode(itemCall)
        return {
            childBountyId,
            parentBountyId,
        }
...
export function getUnassingCuratorData(itemCall: any): UnassingCuratorData {
    if (unassignCurator.v2000000.is(itemCall)) {
        const { parentBountyId, childBountyId } = unassignCurator.v2000000.decode(itemCall)
        return {
            parentBountyId,
            childBountyId,
        }
...
export function getProposeCuratorData(itemCall: any): ProposeCuratorData {
    if (proposeCurator.v2000000.is(itemCall)) {
        const { parentBountyId, childBountyId, curator, fee } = proposeCurator.v2000000.decode(itemCall)
        return {
