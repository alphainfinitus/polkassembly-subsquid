...
import { sendGovEvent, createReferendumV2 } from '@polkadot/mappings/utils/proposals'
import { EGovEvent } from '@shared/types'
import * as storage from '@polkadot/types/storage'
import { ss58codec } from '@polkadot/common/tools'
import * as referendaStorage from '@polkadot/types/referenda/storage'
...
export async function getChainStateDelegations(block: any, voter: string, track: number): Promise<{ votes: bigint, capital: bigint } | null> {
    try {
        if (!storage.convictionVoting.votingFor.v9420.is(block)) {
            return null
        }

        // Convert ss58 address to hex for the storage query
        const voterHex = ss58ToHex(voter)
        const votingFor = await storage.convictionVoting.votingFor.v9420.get(block, voterHex, track)
        if (!votingFor) {
            return null
        }
...
async function getReferendumFromStorage(ctx: ProcessorContext<Store>, index: number, block: any): Promise<ReferendumStorageInfo | undefined> {
    try {
        ...
        // Try different storage versions - newest first
        if (referendaStorage.referendumInfoFor.v1007001?.is(block)) {
            referendumInfo = await referendaStorage.referendumInfoFor.v1007001.get(block, index)
        } else if (referendaStorage.referendumInfoFor.v1005001?.is(block)) {
            referendumInfo = await referendaStorage.referendumInfoFor.v1005001.get(block, index)
        } else if (referendaStorage.referendumInfoFor.v1002000?.is(block)) {
            referendumInfo = await referendaStorage.referendumInfoFor.v1002000.get(block, index)
        } else if (referendaStorage.referendumInfoFor.v1000001?.is(block)) {
            referendumInfo = await referendaStorage.referendumInfoFor.v1000001.get(block, index)
        } else if (referendaStorage.referendumInfoFor.v9420?.is(block)) {
            referendumInfo = await referendaStorage.referendumInfoFor.v9420.get(block, index)
        }
