...
import { referendumV2EnactmentBlocks, fellowshipEnactmentBlocks } from '@polkadot/common/originEnactBlock'
...
import referendumV2 from '@polkadot/mappings/referendumV2'
import { ProcessorContext } from '@src/processor'
import { EGovEvent } from '@shared/types'
import * as preimageStorage from '@polkadot/types/preimage/storage'
...
async function getPreimageStatusData(ctx: ProcessorContext<Store>, hash: string, header: any): Promise<any | undefined> {
    try {
        if (preimageStorage.statusFor.v9340?.is(header)) {
            return await preimageStorage.statusFor.v9340.get(header, hash as any)
        } else if (preimageStorage.statusFor.v9170?.is(header)) {
            return await preimageStorage.statusFor.v9170.get(header, hash as any)
        }
        return undefined
    } catch (e) {
...
async function getPreimageRequestStatusData(ctx: ProcessorContext<Store>, hash: string, header: any): Promise<any | undefined> {
    try {
        if (preimageStorage.requestStatusFor?.v1001002?.is(header)) {
            return await preimageStorage.requestStatusFor.v1001002.get(header, hash as any)
        }
        return undefined
    } catch (e) {
...
export async function sendNotification(ctx: ProcessorContext<Store>, proposal: Proposal, trigger: String) {
    ...
    const notification = {
        trigger: trigger,
        args: {
            network: chainConfigs.polkadot.name,
            postType: type,
